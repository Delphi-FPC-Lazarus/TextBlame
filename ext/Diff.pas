...
type
  ...
  P8Bits = PByte;
  PDiags = ^TDiags;
  TDiags = array [-MAX_DIAGONAL .. MAX_DIAGONAL] of IntType;

  PIntArray = ^TIntArray;
  TIntArray = array[0 .. MAXINT div sizeof(IntType) -1] of IntType;
  PChrArray = ^TChrArray;
  ...
  private
    ...
    procedure InitDiagArrays(MaxOscill, len1, len2: IntType);
    //nb: To optimize speed, separate functions are called for either
    //IntType or character compares ...
    procedure RecursiveDiffChr(offset1, offset2, len1, len2: IntType);
    procedure AddChangeChrs(offset1, range: IntType; ChangeKind: TChangeKind);
    procedure RecursiveDiffInt(offset1, offset2, len1, len2: IntType);
    procedure AddChangeInts(offset1, range: IntType; ChangeKind: TChangeKind);

    function GetCompareCount: IntType;
    function GetCompare(index: IntType): TCompareRec;
  public
    constructor Create(aOwner: TComponent); override;
    destructor Destroy; override;

    //compare either and array of characters or an array of IntTypes ...
    function Execute(pints1, pints2: Pointer; len1, len2: IntType): boolean; overload;
    function Execute(pchrs1, pchrs2: PChar; len1, len2: IntType): boolean; overload;
    ...
    property Count: IntType read GetCompareCount;
    property Compares[index: IntType]: TCompareRec read GetCompare; default;
...
function TDiff.Execute(pchrs1, pchrs2: PChar; len1, len2: IntType): boolean;
var
  maxOscill, x1,x2, savedLen: IntType;
...
begin
  ...
  try
    ...
    begin
      ...
      GetMem(fDiagBuffer, sizeof(IntType)*(maxOscill*2+3));
      GetMem(bDiagBuffer, sizeof(IntType)*(maxOscill*2+3));
      try
...
function TDiff.Execute(pints1, pints2: Pointer; len1, len2: IntType): boolean;
var
  maxOscill, x1,x2, savedLen: IntType;
  compareRec: PCompareRec;
begin
...
procedure TDiff.InitDiagArrays(MaxOscill, len1, len2: IntType);
var
  diag: IntType;
begin
  inc(maxOscill); //for the extra diag at each end of the arrays ...
  P8Bits(fDiag) := P8Bits(fDiagBuffer) - sizeof(IntType)*(MAX_DIAGONAL-maxOscill);
  P8Bits(bDiag) := P8Bits(bDiagBuffer) - sizeof(IntType)*(MAX_DIAGONAL-maxOscill);
...
procedure TDiff.RecursiveDiffChr(offset1, offset2, len1, len2: IntType);
var
  diag, lenDelta, Oscill, maxOscill, x1, x2: IntType;
begin
...
procedure TDiff.RecursiveDiffInt(offset1, offset2, len1, len2: IntType);
var
  diag, lenDelta, Oscill, maxOscill, x1, x2: IntType;
begin
  //nb: the possible depth of recursion here is most unlikely to cause
...
function TDiff.GetCompareCount: IntType;
begin
  result := fCompareList.count;
end;
//------------------------------------------------------------------------------

function TDiff.GetCompare(index: IntType): TCompareRec;
begin
  result := PCompareRec(fCompareList[index])^;
end;
//------------------------------------------------------------------------------

procedure TDiff.AddChangeChrs(offset1, range: IntType; ChangeKind: TChangeKind);
var
  i,j: IntType;
  compareRec: PCompareRec;
begin
...
procedure TDiff.AddChangeInts(offset1, range: IntType; ChangeKind: TChangeKind);
var
  i,j: IntType;
  compareRec: PCompareRec;
begin
